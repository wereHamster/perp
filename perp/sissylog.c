...
static
void logline_post(const char *logline, int flag_continue)
{
   static int     priority;
...
int main(int argc, char *argv[])
{
   ...
   if (e) {
      cstr_vcopy(attention, "notice: ", progname,
                 ": terminating on i/o error reading stdin");
   } else {
      cstr_vcopy(attention, progname,
                 ": terminating normally on eof reading stdin");
   }
   logline_post(attention, 0);
   closelog();
