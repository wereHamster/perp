...
static const char prog_usage[] =
    " [-hV] [-k numkeep] [-r] [-s logsize] [-t] [-z] dir";
...
static void    stamp8601_make(char *stamp8601);
...
static void    tinylog_keep(struct tinylog *tinylog, const char *filename,
                            const char *ext);
static int     tinylog_prune(struct tinylog *tinylog);
static int     tinylog_gzip(struct tinylog *tinylog, const char *file);
static void    tinylog_post(struct tinylog *tinylog, char *logline,
                            size_t len);
...
int write_all(int fd, void *buf, size_t len)
{
   ...
   while (len) {
      do {
         ...
         if ((w == -1) && (errno == EAGAIN)) {
            int            terrno = errno;
            tain_t         t = tain_INIT(1, 0);
            warn_syserr("pausing for write() error");
            tain_pause(&t, NULL);
            errno = terrno;
         }
      ...
      if (w == -1)
         return -1;             /* error! */
      if (w == 0);              /* XXX, no bytes written (shouldn't happen?) */
...
/* stamp8601_make()
** generate an rfc8601-type ascii timestamp into buffer stamp
**
** notes:
**   caller must supply stamp of sufficient size: (sizeof stamp8601)
**   stamp is not nul-terminated
*/
static
void stamp8601_make(char *stamp)
{
   char          *s = stamp;
   tain_t         now;
   time_t         now_utc;
   ...
   nfmt_uint32_pad0_(s, 1900 + tm_now->tm_year, 4);
   s += 4;
   /* use mday format (day of month): */
   nfmt_uint32_pad0_(s, 1 + tm_now->tm_mon, 2);
   s += 2;
   nfmt_uint32_pad0_(s, tm_now->tm_mday, 2);
   s += 2;
   *s = 'T';
   ++s;
   nfmt_uint32_pad0_(s, tm_now->tm_hour, 2);
   s += 2;
   nfmt_uint32_pad0_(s, tm_now->tm_min, 2);
   s += 2;
   nfmt_uint32_pad0_(s, tm_now->tm_sec, 2);
   s += 2;
   *s = '.';
   ++s;
...
static
void init_logdir(struct tinylog *tinylog)
{
   int            fd;
   ...
   if ((fd = pidlock_set(PIDLOCK, mypid, PIDLOCK_NOW)) == -1) {
      fatal_syserr("failure acquiring lock file ", PIDLOCK,
...
*/
static
void init_current(struct tinylog *tinylog, int resume)
{
   struct stat    sb;
   int            e;
   int            fd;
...
*/
static
void tinylog_rotate(struct tinylog *tinylog)
{
   ...
   RETRY(((fd =
           open("current", O_WRONLY | O_CREAT | O_APPEND | O_NONBLOCK,
                0600)) == -1), "failure open() for new current");

   /* mode for active logging: */
   RETRY((fchmod(fd, 0644) == -1), "failure fchmod() for new current");
   fd_cloexec(fd);
...
*/
static
void tinylog_keep(struct tinylog *tinylog, const char *log, const char *ext)
{
   struct stat    sb;
   tain_t         t;
   int            e;
   ...
   RETRY((unlink(log) == -1), "failure unlink() on file ", log);
...
*/
static
int tinylog_prune(struct tinylog *tinylog)
{
   DIR           *dir;
   struct dirent *d;
   char           target[80] = "_z";
...
*/
static
int tinylog_gzip(struct tinylog *tinylog, const char *file)
{
   char           gzipfile[80] = "";
   pid_t          pid;
   int            wstat;

   RETRY(((pid = fork()) == -1), "fail fork() for gzip process");
   ...
   if (pid == 0) {              /* child */
      ...
      if ((fd =
           open("zipped", O_WRONLY | O_CREAT | O_TRUNC | O_NONBLOCK,
                0600)) == -1) {
         return -1;
      }
      fd_move(1, fd);
   ...
   /* parent */
   RETRY((waitpid(pid, &wstat, 0) == -1), "fail waitpid() for gzip process");
   ...
   RETRY((unlink(file) == -1), "fail unlink() after gzip on ", file);
...
static
void tinylog_post(struct tinylog *tinylog, char *logline, size_t len)
{
   int            e;
   ...
   /* post logline: */
   e = write_all(tinylog->fd_current, logline, len);
   if (e != 0) {
      /* XXX */
      warn_syserr("write() error on current");
      return;
   }
#if 0
...
static
int do_log(struct tinylog *tinylog)
{
...
int main(int argc, char *argv[])
{
   ...
   while ((opt = nextopt(&nopt))) {
      ...
      case 'V':
         ...
         break;
      case 'k':
         n = nscan_uint32(nopt.opt_arg, &nscan_end);
         if (*nscan_end != '\0') {
