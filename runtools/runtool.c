...
static const char prog_usage[] =
    ...
    " [-s]"
    " [-S altpath]"
    " [-u user]" " [-U user]" " [-z secs]" " [-W]" " program [args ...]";
...
static
void do_setlock(const char *arg, pid_t pid)
{
...
static
void arglist_add(const char *arg)
{
   char          *newarg;
...
static
void do_argfile(const char *optc, const char *argfile)
{
   int            fd;
   int            eof = 0;
   ioq_t          q;
   ...
   while (!eof) {
      ...
      r = ioq_getln(&q, &L);
      if (r == -1) {
         fatal_syserr("error reading argument file for option -", optc, " ",
                      argfile);
      }
      if (r == 0) {
...
static
void do_envfile(const char *optc, const char *arg)
{
   int            fd;
   int            eof = 0;
   ioq_t          q;
...
static
void do_envdir(const char *optc, const char *arg)
{
   int            fd_orig;
   DIR           *dir;
   struct dirent *d;
   ...
   if (fd_orig == -1) {
      fatal_syserr("unable to open current directory (option -", optc, " ",
                   arg, ")");
   }
   err = chdir(arg);
   if (err == -1) {
   ...
   for (;;) {
      ...
      if (fd == -1) {
         fatal_syserr("unable to open ", arg, "/", d->d_name, " (option -",
                      optc, " ", arg, ")");
      }
      ...
      err = ioq_getln(&q, &L);
      if (err == -1) {
         fatal_syserr("unable to read ", arg, "/", d->d_name, " (option -",
                      optc, " ", arg, ")");
      }
      close(fd);
   ...
   closedir(dir);
   if (fchdir(fd_orig) == -1) {
      fatal_syserr("failure changing to original directory (option -", optc,
                   " ", arg, ")");
   }
   close(fd_orig);
   return;
...
static
void do_env(const char *optc, const char *arg)
{
   struct stat    sb;

   if (stat(arg, &sb) == -1) {
      fatal_syserr("failure stat() on path argument for option -", optc, ": ",
                   arg);
   }
   ...
   if (S_ISREG(sb.st_mode)) {
      ...
      fatal_usage("argument is neither file nor directory for option -", optc,
                  ": ", arg);
   }
...
static
void do_envuid(const char *user)
{
...
static
void runlimit_set(const char *rdef, const char *arg)
{
   int            r;
   struct rlimit  rlim;
   uint32_t       u;
...
static
void do_runlimit(const char *arg_rlimit)
{
...
static
void do_setuid(const char *user)
{
   struct passwd *pw = NULL;
   uid_t          uid;
   gid_t          gid;
...
*/
static
void fdset_redirect(int fd, char op, const char *target)
{
   int            fdx = -1;
   char           t = target[0];
   ...
   case '>':                   /* output redirection, clobber */
      if ((fdx =
           open(target, O_WRONLY | O_CREAT | O_APPEND | O_TRUNC,
                0600)) == -1) {
...
static
void fdset_duplicate(int fd, char target)
{
   int            fdx;
...
void do_fdset(const char *arg_fdset)
{
   ...
   parse instructions from argument string in the format:
       ...
       <pathname >, (must begin with '.' or '/');
 or "%", a special string interpreted as "/dev/null" '=':
 duplication operator, target is:
   single ascii numeral, 0. .9;
 or "!", instruction to close < fd > concatenation:
 multiple instructions may be concatenated with ':' examples:
   "0</dev/null" redirect stdin reading from / dev / null
      ...
      if (op == '=') {
         ...
         if (target[1] != '\0') {
            fatal_usage("bad duplication target in fdset argument ",
                        arg_fdset);
         }
...
*/
static
void do_umask(const char *arg)
{
   const char    *nscan_end;
   mode_t         mode;

   mode = (mode_t) nscan_uint32o(arg, &nscan_end);
   if (*nscan_end != '\0') {
...
void do_sleep(const char *arg)
{
   ...
   secs = nscan_uint32(arg, &nscan_end);
   if (*nscan_end != '\0') {
...
int main(int argc, char *argv[], char *envp[])
{
   ...
   while ((opt = nextopt(&nopt))) {
      ...
      case 'V':
         ...
         break;
      case '0':
         arg_alias = nopt.opt_arg;
         break;
      case 'a':
         opt_arginsert = 0;
         arg_argfile = nopt.opt_arg;
         break;
      case 'A':
         opt_arginsert = 1;
         arg_argfile = nopt.opt_arg;
         break;
      case 'c':
         arg_chdir = nopt.opt_arg;
         break;
      case 'C':
         arg_chroot = nopt.opt_arg;
         break;
      case 'd':
         opt_detach = 1;
         opt_setsid = 1;
      ...
      case 'E':
         ...
         break;
      case 'F':
         arg_fdset = nopt.opt_arg;
         break;
      case 'L':
         arg_setlock = nopt.opt_arg;
         break;
      case 'm':
         arg_umask = nopt.opt_arg;
         break;
      case 'P':
         arg_pidlock = nopt.opt_arg;
         break;
      case 'R':
         arg_rlimit = nopt.opt_arg;
         break;
      case 's':
         opt_setsid = 1;
         break;
      case 'S':
         arg_search = nopt.opt_arg;
         break;
      case 'u':
         arg_setuid = nopt.opt_arg;
         break;
      case 'U':
         arg_envuid = nopt.opt_arg;
         break;
      case 'W':
         warn = 1;
         break;
      case 'z':
         arg_sleep = nopt.opt_arg;
         break;
      case ':':
   ...
   if (arg_setlock && arg_pidlock) {
      fatal_usage
          ("options -L (setlock) and -P (pidlock) are mutually exclusive");
   }
   ...
   newenv_exec(prog,
               ...
               arg_search, (opt_envmerge ? envp : NULL));
